...
for n in range(100):
    ...
    if ret == True:
        ...
        Cb = (0.4997*frame[:, :, 0]-0.33107 *
              frame[:, :, 1]-0.16864*frame[:, :, 2])
        ...
        for yblock in range(30):  # (480-8)/8
            # print("yblock=",yblock)
            block[0] = yblock*8+130  # 200
            for xblock in range(30):  # (640-8)/8
                # print("xblock=",xblock)
                block[1] = xblock*8+220  # 300
                # current block:
                Yc = Y[block[0]:block[0]+8, block[1]
                    :block[1]+8]  # Current frame
                # previous block:
                Yp = Yprev[block[0]-8: block[0]+8,
                           block[1]-8: block[1]+8]  # Previous frame
                ...
                # Index2d will have the pixel index where the block has maximum correlation
...
############################################## between the current and the previous block hence motion estimation #######################################################################
                if np.argmax(Ycorr) >= bestmae:
                    mv = np.add(block, index2d)
                    cv2.line(framevectors, tuple(block)[
                             ::-1], tuple(mv)[::-1], (0, 255, 255), 2)
                    # print('here')
